...
class Infocontroller extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        if ($request->hasFile('img'))
         ...
         $data['depict'] = trim($data['depict']);
         //执行添加
         $res = DB::table('hkyl_info') -> insert($data);
         if($res)
         {
         	return redirect('/admin/info/index') -> with(['info'=>'添加成功']);
    ...
    {     
        ...
        ->where('pid','=','11')
        ->get();
        return view('admin.info.index',['data' => $data,'cate'=>$cate,'request' => $request->all()]);
    }
    //edit
    public function edit($id)
    {
        $data = DB::table('hkyl_info') -> where('id',$id) ->first();
        $cate = DB::table('hkyl_cate') -> where('id',$data -> cid) ->first();
        $res  = DB::table('hkyl_cate') -> where('pid',$cate ->  pid) -> get();
        
        return view('admin.info.edit',['data'=>$data,'res'=>$res]);
    }
    //update
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('img'))
        {
            ...
            {   
                 unset($data['pic']);
                $img = DB::table('hkyl_info') -> where('id',$data['id']) -> first() -> img;
        ...
        if(empty($data['news']))
        {
            ...
            $res = DB::table('hkyl_info') -> where('id',$data['id']) -> update($data);
             if($res)
             {
                return redirect('/admin/info/index') -> with(['info'=>'修改成功']);
    ...
    {  
        $img = DB::table('hkyl_info') -> where('id',$id) ->first() ->img;
        $oldimg = './updates/'.$img;
        $res = DB::table('hkyl_info') -> delete($id);
        if($res)
         {   
            if(file_exists($oldimg))
            {
            ...
            return redirect('/admin/info/index') -> with(['info'=>'删除成功']);

...
class Traincontroller extends Controller
{
    ...
    public function add()
    {
        ...
        ->where('pid','=','3')
        ->get();
         return view('admin.train.add',['data'=>$data]);
    ...
    public function insert(Request $request)
    {
        ...
        if ($request->hasFile('img'))
         ...
         $data['depict'] = trim($data['depict']);
         //执行添加
         $res = DB::table('hkyl_train') -> insert($data);
         if($res)
         {
         	return redirect('/admin/train/index') -> with(['info'=>'添加成功']);
    ...
    {     
        ...
        return view('admin.train.index',['data' => $data,'cate'=>$cate,'request' => $request->all()]);
    }
    //edit
    public function edit($id)
    {
        $data = DB::table('hkyl_train') -> where('id',$id) ->first();
        $cate = DB::table('hkyl_cate') -> where('id',$data -> cid) ->first();
        $res  = DB::table('hkyl_cate') -> where('pid',$cate ->  pid) -> get();
        
        return view('admin.train.edit',['data'=>$data,'res'=>$res]);
    }
    //update
    public function update(Request $request)
    {
        ...
        if(empty($data['news']))
        {
            ...
            $res = DB::table('hkyl_train') -> where('id',$data['id']) -> update($data);
             if($res)
             {
                return redirect('/admin/train/index') -> with(['info'=>'修改成功']);
    ...
    {  
        $img = DB::table('hkyl_train') -> where('id',$id) ->first() ->img;
        $oldimg = './updates/'.$img;
        $res = DB::table('hkyl_train') -> delete($id);
        if($res)
         {   
            if(file_exists($oldimg))
            {

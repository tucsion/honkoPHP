...
class InfoController extends Controller
{
    ...
    {   
      ...
      $xuanzhe = 8;
      //页面数据
      $wz = DB::table('hkyl_info') -> where('cid','=',$id) -> orderBy('addtime') -> paginate(5);
      $wzhang = DB::table('hkyl_info') -> where('cid','=',$id) -> orderBy('addtime') -> first();
      
      
    	return view('home.info.index',['set' => $set,'huiyi'=>$huiyi,'qiye'=>$qiye,'wz'=>$wz,'jiaoyu'=>$jiaoyu,'wzhang'=>$wzhang,'id'=>$id,'fenlei'=>$fenlei,'yangsheng'=>$yangsheng,'banner' => $banner,'expert' => $expert,'train' => $train,'ys' => $ys,'hickey'=>$hickey,'procure'=>$procure,'goods'=>$goods,'health'=>$health,'xuanzhe' => $xuanzhe]);
    }
    ...
    {   
      ...
      $banner = DB::table('hkyl_banner') -> where('id','=','4') -> first();
      $jiaoyu = DB::table('hkyl_cate') -> where('pid','=','3') -> orderBy('id') -> first();
      $detail  = DB::table('hkyl_info') -> where('id','=',$id)-> first();
      ...
      return view('home.info.detail',['set' => $set,'qiye'=>$qiye,'huiyi'=>$huiyi,'yangsheng'=>$yangsheng,'shang'=>$shang,'xia'=>$xia,'detail'=>$detail,'id'=>$id,'fenlei'=>$fenlei,'jiaoyu'=>$jiaoyu,'banner' => $banner,'expert' => $expert,'train' => $train,'ys' => $ys,'hickey'=>$hickey,'procure'=>$procure,'goods'=>$goods,'health'=>$health,'xuanzhe' => $xuanzhe]);
    }
